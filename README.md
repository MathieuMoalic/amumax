...
Here we have an emitting CPW antenna and a receiving one further down the waveguide. We use the `magModulatedByMask` to extract the signal generated by the spin waves.
```go
emitter := cpwVectorMask(current,wire_width, wire_height, wire_distance, emitter_offset, wire_z_offset)
receiver := cpwVectorMask(current,wire_width, wire_height, wire_distance, receiver_offset, wire_z_offset)

B_ext.add(emitter, amplitude * sin(2*pi*f*t))

TableAddVar(magModulatedByMask(receiver), "S23", "") 
tableAutoSave(sampling_interval)
autosave(B_ext, sampling_interval)
autosave(m, sampling_interval)
run(2000 * sampling_interval)
```
