...
func Resize(dst, src *data.Slice, layer int) {
	...
	log_old.AssertMsg(dst.NComp() == 1 && src.NComp() == 1, "Component mismatch: dst and src must both have 1 component in Resize")

	scalex := srcsize[X] / dstsize[X]
	scaley := srcsize[Y] / dstsize[Y]
	log_old.AssertMsg(scalex > 0 && scaley > 0, "Scaling factors must be positive in Resize")
