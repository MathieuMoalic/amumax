...
//
// see exchange.cu
func AddExchange(B, m *data.Slice, Aex_red SymmLUT, Msat MSlice, regions *Bytes, mesh mesh2.MeshLike) {
	c := mesh.CellSize()
...
func ExchangeDecode(dst *data.Slice, Aex_red SymmLUT, regions *Bytes, mesh mesh2.MeshLike) {
	c := mesh.CellSize()
	wx := float32(2 / (c[X] * c[X]))
	wy := float32(2 / (c[Y] * c[Y]))
	wz := float32(2 / (c[Z] * c[Z]))
