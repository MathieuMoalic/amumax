...
func ShiftX(dst, src *slice.Slice, shiftX int, clampL, clampR float32) {
...
func ShiftEdgeCarryX(dst, src, othercomp, anothercomp *slice.Slice, shiftX int, clampL, clampR float32) {
...
func ShiftY(dst, src *slice.Slice, shiftY int, clampL, clampR float32) {
...
func ShiftEdgeCarry(dst, src, othercomp, anothercomp *slice.Slice, shiftY int, clampL, clampR float32) {
...
func ShiftZ(dst, src *slice.Slice, shiftZ int, clampL, clampR float32) {
...
func ShiftBytes(dst, src *Bytes, m *mesh.Mesh, shiftX int, clamp byte) {
	N := m.Size()
	cfg := make3DConf(N)
	k_shiftbytes_async(dst.Ptr, src.Ptr, N[X], N[Y], N[Z], shiftX, clamp, cfg)
}

func ShiftBytesY(dst, src *Bytes, m *mesh.Mesh, shiftY int, clamp byte) {
	N := m.Size()
	cfg := make3DConf(N)
