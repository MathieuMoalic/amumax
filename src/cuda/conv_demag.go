...
type DemagConvolution struct {
	...
	fftRBuf          [3]*slice.Slice    // FFT input buf; 2D: Z shares storage with X.
	fftCBuf          [3]*slice.Slice    // FFT output buf; 2D: Z shares storage with X.
	kern             [3][3]*slice.Slice // FFT kernel on device
...
func NewDemag(inputSize, PBC [3]int, kernel [3][3]*slice.Slice, test bool) *DemagConvolution {
...
func (c *DemagConvolution) Exec(B, m, vol *slice.Slice, Msat MSlice) {
...
func (c *DemagConvolution) exec3D(outp, inp, vol *slice.Slice, Msat MSlice) {
...
func (c *DemagConvolution) exec2D(outp, inp, vol *slice.Slice, Msat MSlice) {
...
func zero1_async(dst *slice.Slice) {
	cu.MemsetD32Async(cu.DevicePtr(uintptr(dst.DevPtr(0))), 0, int64(dst.Len()), stream0)
}

// forward FFT component i
func (c *DemagConvolution) fwFFT(i int, inp, vol *slice.Slice, Msat MSlice) {
...
func (c *DemagConvolution) bwFFT(i int, outp *slice.Slice) {
	c.bwPlan.ExecAsync(c.fftCBuf[i], c.fftRBuf[i])
	out := outp.Comp(i)
	copyUnPad(out, c.fftRBuf[i], c.inputSize, c.realKernSize)
}

func (c *DemagConvolution) init(realKern [3][3]*slice.Slice) {
	// init device buffers
	...
	output := c.fftCBuf[0]
	input := c.fftRBuf[0]
	fftKern := slice.NewSlice(1, physKSize)
	kfull := slice.NewSlice(1, output.Size()) // not yet exploiting symmetry
	kfulls := kfull.Scalars()
	kCSize := physKSize
	kCSize[X] *= 2                      // size of kernel after removing Y,Z redundant parts, but still complex
	kCmplx := slice.NewSlice(1, kCSize) // not yet exploiting X symmetry
	kc := kCmplx.Scalars()
	...
	for i := 0; i < 3; i++ {
		for j := i; j < 3; j++ { // upper triangular part
			if realKern[i][j] != nil { // ignore 0's
				// FW FFT
				slice.Copy(input, realKern[i][j])
				c.fwPlan.ExecAsync(input, output)
				slice.Copy(kfull, output)
