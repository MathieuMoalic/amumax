...
func corePos() []float64 {
	...
	pos[X] = float64(maxX) + interpolateMaxpos(
		max, -1, abs(mz[maxY][maxX-1]), 1, abs(mz[maxY][maxX+1])) -
		float64(Nx)/2 + 0.5
	pos[Y] = float64(maxY) + interpolateMaxpos(
	...
	pos[X] *= c[X]
	pos[Y] *= c[Y]
	pos[Z] = float64(mZ[maxZ][maxY][maxX]) // 3rd coordinate is core polarization

	pos[X] += getShiftPos() // add simulation window shift
	return pos
}

func interpolateMaxpos(f0, d1, f1, d2, f2 float32) float64 {
	b := (f2 - f1) / (d2 - d1)
