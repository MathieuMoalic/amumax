...
func init() {
	...
	DeclFunc("magModulatedByMaskGPU", magModulatedByMaskGPU, "magModulatedByMaskGPU")
	DeclFunc("cpwVectorMask", cpwVectorMask, "cpwVectorMask")
	DeclFunc("cpwVectorMaskGPU", cpwVectorMaskGPU, "cpwVectorMaskGPU")
}
...
func fieldFromWireVectorMask(I, width, height, xcenter, zcenter float64) *data.Slice {
	Nx, Ny, Nz := Mesh.GetNi()
	maskSlice := cuda.NewSlice(3, [3]int{Nx, Ny, Nz})
	for ix := range Nx {
		for iy := range Ny {
			for iz := range Nz {
				...
				maskSlice.Set(0, ix, iy, iz, Bx)
				maskSlice.Set(1, ix, iy, iz, By)
				maskSlice.Set(2, ix, iy, iz, Bz)
			}
		}
	}
	return maskSlice
...
func cpwVectorMask(I, width, height, distance, xoffset, zoffset float64) *data.Slice {
	Nx, Ny, Nz := Mesh.GetNi()
	maskSlice := newVectorMask(Nx, Ny, Nz)
	for ix := range Nx {
		for iy := range Ny {
			for iz := range Nz {
				r := index2Coord(ix, iy, iz)
...
func cpwVectorMaskGPU(I, width, height, distance, xoffset, zoffset float64) *data.Slice {
	mask := cpwVectorMask(I, width, height, distance, xoffset, zoffset)
	return cuda.GPUCopy(mask)
}
...
func magModulatedByMaskGPU(maskSlice *data.Slice) float64 {
	magSlice := NormMag.Buffer()
	s := cuda.Dot(magSlice, maskSlice) // sum over c,x,y,z of mag[c]*mask[c]
	return float64(s)
}
