...
func randomDir(rng *rand.Rand) data_old.Vector {
...
func vortex(circ, pol int) config {
	...
	return func(x, y, z float64) data_old.Vector {
		...
		return noNaN(data_old.Vector{mx, my, mz}, pol)
	}
}
...
func noNaN(v data_old.Vector, pol int) data_old.Vector {
...
func conical(q, coneDirection data_old.Vector, coneAngle float64) config {
	...
	ua := data_old.Vector{u[X] * u[Z], u[Y] * u[Z], u[Z]*u[Z] - 1}.Div(p)
	ub := data_old.Vector{-u[Y], u[X], 0}.Div(p)
...
func helical(q data_old.Vector) config {
...
func (c config) RotZ(θ float64) config {
	...
	return func(x, y, z float64) data_old.Vector {
		...
		mx_ := m[X]*cos - m[Y]*sin
		my_ := m[X]*sin + m[Y]*cos
		return data_old.Vector{mx_, my_, m[Z]}
	}
}
