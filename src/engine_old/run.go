...
func setSolver(typ int) {
	...
	switch typ {
	default:
		log_old.Log.ErrAndExit("SetSolver: unknown solver type:  %v", typ)
	case BACKWARD_EULER:
...
func setMaxTorque(τ *data_old.Slice) {
	LastTorque = cuda_old.MaxVecNorm(τ)
}
...
func adaptDt(corr float64) {
	...
	log_old.AssertMsg(Dt_si > 0, fmt.Sprint("Time step too small: ", Dt_si))
}
...
func checkExchangeLenght() {
	...
	for _, region := range Regions.GetExistingIndices() {
		...
		if !exchangeLenghtWarned {
			...
			if Mesh.Dy > lex {
				log_old.Log.Warn("Warning: Exchange length (%.3g nm) smaller than dy (%.3g nm) in region %d", lex*1e9, Mesh.Dy*1e9, region)
				exchangeLenghtWarned = true
			}
			if Mesh.Dz > lex && Mesh.Nz > 1 {
				log_old.Log.Warn("Warning: Exchange length (%.3g nm) smaller than dz (%.3g nm) in region %d", lex*1e9, Mesh.Dz*1e9, region)
				exchangeLenghtWarned = true
			}
		}
