{
  ...
  outputs = {nixpkgs, ...}: let
    ...
    GitFrontend = buildFrontend {
      src = ./frontend;
      npmDepsHash = "sha256-VbtmgsoxI0and+L43RqKgm0aeQsJDf9SzKnM+jOMR+8=";
      version = gitVersion;
    };
    ...
    #################### RELEASE ########################
    releaseVersion = "2024.11.20"; # Set the version for the Release build

    ReleaseSrc = pkgs.fetchFromGitHub {
      owner = "MathieuMoalic";
      repo = "amumax";
      rev = releaseVersion;
      hash = "sha256-CVEaadFEx0mNuEklQoP1eXTgPgiKWeI1NBBMAm1xFYI=";
    };
