{
  ...
  outputs = {nixpkgs, ...}: let
    ...
    GitFrontend = buildFrontend {
      src = ./frontend;
      npmDepsHash = "sha256-CVXMBuadgjPWFf2T4pJiyJbcuACp++XYOUvNDeP5dPY=";
      version = gitVersion;
    };
    GitBuildAmumax = with pkgs.lib.fileset;
      buildAmumax {
        ...
        vendorHash = "sha256-AprYnDIbSJaHM7XUMfaEVDvAxQ09N4ns+aUHfhW53sw=";
        version = gitVersion;
      };

    #################### RELEASE ########################
    releaseVersion = "2024.10.23"; # Set the version for the Release build

    ReleaseSrc = pkgs.fetchFromGitHub {
      owner = "MathieuMoalic";
      repo = "amumax";
      rev = releaseVersion;
      hash = "sha256-lGCsbnmThXGTIP72cScE4c24uKMtNMcMvMtLoKVJa+I=";
    };
